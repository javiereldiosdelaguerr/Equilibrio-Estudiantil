"""
observability.py  –  Logs estructurados + métricas de latencia
Cópialo en CADA carpeta de microservicio (gateway, moods, usuarios, etc.)
"""
...
class CircuitBreaker:
    ...
    - CLOSED   : tráfico normal; cuenta fallos.
    - OPEN     : bloquea llamadas durante `recovery_timeout` segundos.
    - HALF_OPEN: deja pasar UNA llamada de prueba; si falla vuelve a OPEN.
    """
...
# ──────────────────────────────────────────────
#  MÉTRICAS DE LATENCIA (en memoria)
# ──────────────────────────────────────────────
...
def init_observability(app, service_name: str):
    """
    Llama esto en el `app.py` de cada servicio:
    ...
    Agrega automáticamente:
      • Logs de cada request (método, ruta, status, latencia)
      • Ruta GET /metrics/latency con el resumen de latencias
    """
